...
class ActivityLogListView(SortableMixin, ListView):
    ...
    def get_context_data(self, **kwargs):
        ...
        # Users who have activity log entries
        user_ids = ActivityLog.objects.values_list("user_id", flat=True).distinct()
        context["users"] = User.objects.filter(id__in=user_ids).order_by("first_name", "last_name")
        return context
...
class SchoolActivityListView(ListView):
    ...
    def get_queryset(self):
        ...
        return (
            ActivityLog.objects.filter(related_school_id=school_id)
...
class CourseActivityListView(ListView):
    ...
    def get_queryset(self):
        ...
        return (
            ActivityLog.objects.filter(related_course_id=course_id)
            .select_related("user", "content_type")
            .order_by("-timestamp")
        )
