...
class Command(BaseCommand):
    help = 'Backup database and media files to Backblaze B2 or local storage'
        ...
        if not local_only:
            ...
            if not s3:
                self.stdout.write(self.style.WARNING(
                    'B2 settings missing, using local-only mode'
                ))
                local_only = True
    ...
    def _get_b2_client(self):
        ...
        try:
            ...
            self.stderr.write(self.style.WARNING(f'Failed to create B2 client: {e}'))
            return None
    ...
    def _backup_database(self, local_backup_dir, s3, backup_name, local_only):
        ...
        try:
            ...
            if not local_only and s3:
                ...
                self.stdout.write(self.style.SUCCESS(f'  Uploaded to B2: {backup_name}/{db_filename}'))
    ...
    def _backup_media(self, local_backup_dir, s3, backup_name, local_only):
        ...
        try:
            ...
            if not local_only and s3:
                ...
                self.stdout.write(self.style.SUCCESS(f'  Uploaded to B2: {backup_name}/{media_filename}'))
    ...
    def _cleanup_b2_backups(self, s3, retention_days):
        ...
        try:
            ...
            for key in old_objects:
                ...
                self.stdout.write(f'  Deleted from B2: {key}')

            if old_objects:
                self.stdout.write(f'Deleted {len(old_objects)} old B2 file(s)')
