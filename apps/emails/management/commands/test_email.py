...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        try:
            ...
            self.stderr.write(self.style.ERROR(f'Skabelon "{email_type}" findes ikke. Kør migrations først.'))
            return
        ...
        if recipient:
            if not settings.RESEND_API_KEY:
                self.stderr.write(
                    self.style.ERROR(
                        "RESEND_API_KEY er ikke konfigureret. " "Tilføj den til .env for at sende e-mails."
                    )
                )
                return
            ...
            self.stdout.write(self.style.WARNING("Brug --to <email> for at sende en test e-mail"))
