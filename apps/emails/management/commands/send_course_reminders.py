                ...
                if dry_run:
                    ...
                    if success:
                        ...
                        self.stderr.write(self.style.ERROR(
                            f'  Fejl ved afsendelse til: {signup.participant_email}'
                        ))

        self.stdout.write('')
        if dry_run:
            self.stdout.write(self.style.WARNING(
                f'[DRY-RUN] Ville sende {total_sent} e-mails, {total_skipped} allerede sendt'
            ))
        else:
            self.stdout.write(self.style.SUCCESS(
                f'Sendt {total_sent} e-mails, {total_skipped} sprunget over (allerede sendt)'
            ))
